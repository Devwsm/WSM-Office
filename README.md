...
> **Status kode (dicek 2026-09-22):** 370 tes otomatis lulus (4.116 assertion, PHP 8.3.6, SQLite), 158 route, 42 migrasi — dijalankan ulang setelah `composer update` (Symfony turun ke v7.4, `maatwebsite/excel` naik ke 4.0.3), tidak ada regresi. Isi dokumen ini dicocokkan dengan kode yang berjalan; kolom "Prototype v32" di Bab 2.1 dicocokkan langsung ke `WOS_2_0_STANDALONE_v32.html` (file prototype tidak ada di repo project ini, tapi sudah diperiksa langsung di luar repo). Dua koreksi dari pemeriksaan itu: No. 20 (Executive People Overview) dan No. 28 (Assign/Reminder) — lihat catatan di baris masing-masing.
...
**Ukuran:** 158 route (75 GET, 83 tulis), 42 migrasi, 25 model, 31 FormRequest, 106 view Blade, 4 seeder, 370 tes.
...
| 20  | Executive People Overview                                                                                | **Bukan halaman terpisah** — di kode prototype ini cuma label sidebar (`data-opage="dashboard"`) yang mengarah ke fungsi `ownerDashboard()` yang sama persis dengan No. 15                                                                           | Sudah ada — sama dengan Dashboard Owner (No. 15), memang tidak butuh halaman baru                                                                                                                                                                     | ✅ (= No. 15, cuma beda nama di prototype)                             | Owner, Developer                             |
...
| 28  | Assign / Reminder dari dashboard                                                                         | Form cepat (dari console admin/secretary kerja): pilih PIC, judul, due date, priority — sekali submit langsung (a) bikin Work Tracker task berworkstream "Reminder / Admin" + `isReminder:true`, dan (b) kirim memo notifikasi ke inbox PIC itu juga | Kolom `is_reminder` dan section "REMINDER / ADMIN" sudah ada di `WorkItem`, tapi belum ada form/aksi "Assign Reminder" yang bikin task + memo sekaligus dalam satu submit seperti prototype                                                           | ❌ (sekarang jelas — bisa masuk Bab 4.2, bukan lagi Belum dijadwalkan) | —                                            |
...
**Hitungan status (49 baris):** ✅ 31 · ⚠️ 8 · ❌ 4 · ➕ 6. (No. 20 dipindah dari ❌ ke ✅ setelah dicocokkan ulang ke prototype — lihat catatan di baris itu.)
...
**Hasil terakhir:** 370 tes lulus, 0 dilewati, 4.116 assertion, ±16 detik, di SQLite (PHP 8.3.6) — dikonfirmasi ulang lewat audit independen (16,79 detik). Belum diuji ulang di MySQL/MariaDB asli setelah perubahan terbaru. **Catatan:** ini cuma bisa jalan setelah `vendor/composer/platform_check.php` dilonggarkan manual (menerima PHP 8.3, bukan 8.4.1) — persis Blocker 4.1 no. 1; `vendor/` apa adanya (tanpa dilonggarkan) menolak start sama sekali di PHP 8.3.6, jadi klaim blocker itu terkonfirmasi nyata, bukan cuma teori.
...
routes/
├── web.php                                 # 158 route: publik, /app, /owner, /dashboard, rekrutmen, approval
...
├── Feature/                                # 18 file, 369 tes (lihat tabel Bab 2.2)
...
| Topik                            | Keputusan                                                                                                                                                                                          |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Versi PHP hosting                | cPanel Rumahweb mentok di **PHP 8.3** (tidak ada 8.4). ✅ Sudah dikerjakan 2026-09-22 (composer.json + `composer update`, lihat 4.1 no. 1).                                                        |
...
**Blocker 1 (Versi PHP), selesai 2026-09-22:** `config.platform.php` diset ke `8.3.0` dan konstrain `maatwebsite/excel` disamakan ke `^4.0` di `composer.json`. `composer update` dijalankan di lokal (bukan di lingkungan audit — sempat dicoba di sana tapi `repo.packagist.org` diblokir jaringan): hasilnya `maatwebsite/excel` naik ke 4.0.3, `phpoffice/phpspreadsheet` naik ke 5.10.0, lima paket Symfony turun dari v8.1 ke v7.4 (`clock`, `css-selector`, `event-dispatcher`, `string`, `translation`), dan `ezyang/htmlpurifier` (tidak terpakai) ikut terhapus. Baseline **370 tes lulus (4.116 assertion)** sebelum update, dan **370 tes lulus (4.116 assertion) lagi** setelah update — tidak ada regresi. `vendor/` hasil update ini yang harus ikut di-upload ke server, bukan yang lama.
...
Butir 2–6 adalah keputusan 2026-09-21 ("eksekusi"); butir 7 keputusan sebelumnya; butir 1 sudah selesai. Urutan pengerjaannya ada di 4.4.
...
8. **Assign / Reminder dari dashboard** (Bab 2.1 no. 28, baru jelas setelah dicocokkan ke prototype — sebelumnya di "belum dijadwalkan"): buat form cepat (mis. di halaman Work Control atau IT) yang, dalam satu submit, membuat Work Item berworkstream "Reminder / Admin" (pakai `is_reminder=true` yang sudah ada) sekaligus mengirim memo ke PIC yang dipilih. Kolom datanya sudah siap, tinggal form + aksinya.
...
1. ✅ **Turunkan target PHP ke 8.3** (4.1 no. 1) — selesai 2026-09-22, lihat catatan di Bab 4.1.
...
7. **Assign / Reminder dari dashboard** (4.2 no. 8) — opsional, tidak menahan deploy, bisa disisipkan kapan saja sebelum atau sesudah langkah 8–10.
8. **Perbaiki bug dan keterbatasan yang diketahui** (4.3): kartu People/Recruitment di `/dashboard`, scope export Rekap Absensi, dan pengajuan cuti tumpang tindih.
