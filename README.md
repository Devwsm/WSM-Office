...
> **Status kode (dicek ulang 2026-09-25 terhadap kode, riwayat git, dan log GitHub Actions; Bab 5 Production baru):** 380 tes otomatis lulus (4.156 assertion, PHP 8.3.6, SQLite), 159 route aplikasi (+ `/up`), 42 migrasi, 5 seeder. **`phpunit.xml` sekarang mengunci `WOS_PREVIEW_MODE=true`** (2026-09-25), jadi nilai `WOS_PREVIEW_MODE` di `.env` lokal tidak lagi memengaruhi hasil tes (sebelumnya 376/380 kalau `.env` lokal berisi `WOS_PREVIEW_MODE=false`, lihat riwayat di 5.6). Isi dokumen ini dicocokkan dengan kode yang berjalan. Kolom "Prototype v32" di Bab 2.1 dicocokkan langsung ke source prototype (`WOS_2_0_STANDALONE_v32.html`) pada 2026-09-23; baris Bab 2.1 tidak diaudit ulang satu per satu pada 2026-09-25 (yang dicek ulang: angka, struktur file, klaim deployment, dan butir yang ditandai sudah/belum dikerjakan).
...
| 28  | Assign / Reminder dari dashboard                                                                           | Ada — bagian dari **Work Dashboard** (Secretary/Admin Console), akses khusus jabatan tertentu: assign task, follow-up tracker, blast memo, dan reminder pekerjaan (reminder ikut masuk ke Memos) | Belum ada. Di kode baru ada kolom `is_reminder` pada task dan section "REMINDER / ADMIN"; belum ada aksi khusus di dashboard                                                                                                                          | ❌ → dikerjakan (Bab 4.2 no. 9, dicek ke source prototype 2026-09-23) | —                                            |
...
9. **Assign / Reminder dari dashboard (baru, scope diklarifikasi 2026-09-23 dari source prototype):** buat versi Work Dashboard-nya — jabatan yang diberi akses bisa assign task ke Work Tracker, follow-up tracker, blast memo ke tim, dan bikin reminder pekerjaan yang otomatis masuk ke Memo penerima (mengikuti pola `isReminder` di prototype). Kolom `is_reminder` di task dan section "REMINDER / ADMIN" yang sudah ada di kode dipakai sebagai basisnya. Setelah selesai, perbarui baris no. 28 di Bab 2.1.
...
- **Assign / Reminder dari dashboard** (Bab 2.1 no. 28): sudah jelas scope-nya — bagian dari Work Dashboard (Secretary/Admin Console) untuk jabatan tertentu. Sudah masuk daftar kerja di Bab 4.2 no. 9.
