...
> **Status kode (dicek 2026-09-21):** 322 tes otomatis lulus (3.786 assertion, PHP 8.3.6, SQLite), 156 route, 41 migrasi. Isi dokumen ini dicocokkan dengan kode yang berjalan; kolom "Prototype v32" di Bab 2.1 bersumber dari audit terhadap prototype (file prototype tidak ada di repo).
...
**Konvensi kode:** validasi lewat kelas `FormRequest` (31 kelas di `app/Http/Requests`); `/owner` hanya untuk role `owner`, `/app` untuk semua akun internal; akses berbasis **modul** (`dashboard_access`, level `view`/`manage`) untuk area `/dashboard`, rekap, approval, dan rekrutmen; `throttle` pada login, form publik (kontak, lamaran), absen, pengajuan, balasan memo, buka kunci dashboard, blast memo, dan import; audit log untuk aksi penting (karyawan, akses modul, approval, payroll, pengaturan kantor, import karyawan).

**Ukuran:** 156 route (74 GET, 82 tulis), 41 migrasi, 25 model, 31 FormRequest, 105 view Blade, 3 seeder, 322 tes.
...
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Apa itu            | Label jabatan di akun: `owner`, `manajer`, `hrd`, `karyawan`                                                                     | Per modul: `none` / `view` / `manage` (tabel `dashboard_access`)               |
| Yang ditentukannya | Area `/owner` (hanya Owner); Owner punya akses penuh; Owner dan HRD melihat semua orang di Rekap Absensi. Selebihnya hanya label | Menu dashboard yang muncul, dan boleh ubah data (`manage`) atau tidak (`view`) |
| Diatur di          | Karyawan → Edit → Role                                                                                                           | Karyawan → Akses (hanya Owner)                                                 |

**Persetujuan izin/cuti/lembur/koreksi:** halaman persetujuan hanya bisa dibuka dengan akses modul `people`, tetapi siapa yang boleh memutuskan ditentukan oleh **atasan langsung** karyawan itu (field "Atasan Langsung"), dan Owner boleh memutus semuanya. Cakupan Rekap Absensi untuk manajer adalah dirinya dan seluruh bawahan turunannya.
...
|     | **Area Owner (`/owner`)**                                                                                |                                                                               |                                                                                                                                                                                                 |                                                              |                                              |
| 15  | Dashboard Owner                                                                                          | Ringkasan + ritme mingguan bisa diedit                                        | Ringkasan ada; **ritme mingguan hard-code** di controller (jam WFO-nya diambil dari Pengaturan Kantor)                                                                                          | ⚠️ → dikerjakan (Bab 4.2 no. 3)                              | Owner                                        |
...
| 43  | Export & Import Center (15 export: Excel 12 + PDF 3; 4 import: Work Tracker, Karyawan, KPI, Budget)      | Hanya import Work Tracker                                                     | Ada, berbasis katalog & gerbang modul                                                                                                                                                           | ➕                                                           | Sesuai modul masing-masing                   |
...
**Hitungan status (47 baris):** ✅ 29 · ⚠️ 8 · ❌ 6 · ➕ 4.
...
**Hasil terakhir:** 322 tes lulus, 0 dilewati, 3.786 assertion, ±17 detik, di SQLite (PHP 8.3.6). Belum diuji ulang di MySQL/MariaDB asli setelah perubahan terbaru.
...
| `PageGuideTest`         | 10  | Peta route → panduan valid, tidak ada panduan yatim, semua halaman dashboard tanpa parameter punya tombol, label akses View/Manage/Khusus Owner, teks di-escape                                                    |
...
Tiap halaman dashboard punya tombol **"? Panduan"** (melayang di kanan bawah). Ditekan, muncul modal berisi fungsi halaman, fitur yang bisa dipakai, dan hal yang perlu diketahui. Ditutup lewat ✕, tombol "Mengerti, tutup", tombol Esc, atau klik area gelap, sehingga pengguna tidak meninggalkan halaman. Modal responsif (bottom-sheet di HP, di tengah layar di desktop). Halaman modul juga menampilkan label akses pengguna ("Akses kamu: Manage" atau "View"), dan halaman Owner berlabel "Khusus Owner".

Panduan dicari dari nama route lewat `App\Support\PageGuide`, jadi tidak ada view halaman yang diubah; satu-satunya keterkaitan di layout adalah satu baris `@include` (`partials/page-guide`). Isi: 44 panduan untuk 54 pola route, ditulis dalam bahasa Indonesia dan dicocokkan dengan perilaku kode.
...
├── migrations/                             # 41 migrasi berurutan (users → sesi, absensi, modul dashboard, rekrutmen, kontak, lembur)
...
routes/
├── web.php                                 # 156 route: publik, /app, /owner, /dashboard, rekrutmen, approval
...
├── Feature/                                # 16 file, 321 tes (lihat tabel Bab 2.2)
...
│   ├── PageGuideTest                       # 10 tes panduan halaman dashboard
...
1. **Reset password dari dashboard IT, role developer, dan seeder testing baru.** Keputusan 2026-09-21 sudah lengkap; tinggal dikerjakan. Belum ada satu pun di kode (tidak ada role `developer`, kolom `must_change_password`, maupun halaman reset).
    ...
    - **Role `developer` (Tingkat 2, seperti Owner tanpa tiga hal sensitif).** Bisa: Manage ke 10 modul dashboard (lewat akses modul), lihat absensi semua orang di Rekap, reset password non-Owner, kelola Karyawan (tambah, ubah, nonaktifkan), Struktur Organisasi, Pengaturan Kantor, Pesan Kontak, Dashboard Owner, dan import/export karyawan. **Tidak bisa:** membuat, mengubah, mereset, atau menonaktifkan akun Owner (termasuk menjadikan orang lain Owner), membuka Dashboard Access, dan memutus persetujuan orang yang bukan bawahan langsungnya. Efek samping "lihat semua": ikut melebar ke foto selfie, koreksi absen, dan export rekap karena memakai scope yang sama; Manajer biasa tidak berubah.
    ...
    - **Guard akun Owner:** konversi pelamar sudah dijaga (hanya Owner yang bisa membuat akun Owner). Guard yang sama harus dipasang di form Karyawan dan import karyawan saat role developer dibuat, karena area itu akan terbuka untuk developer (sekarang khusus Owner).
    - **Panduan dan tes:** halaman Reset Password wajib punya panduan (`PageGuideTest` akan gagal kalau belum); label akses di panduan halaman `/owner` berubah dari "Khusus Owner" menjadi "Owner & Developer"; tes baru mencakup kolom `must_change_password`, matriks akses developer, guard akun Owner, aturan reset, dan seeder.
...
- **Tes otomatis:** 322 tes lulus di 17 file (Bab 2.2); jalankan sebelum tiap deploy. Belum ada tes unit murni dan belum ada tes browser (Dusk/Playwright) untuk UI, geolocation, dan kamera.
...
4. **Payroll** (4.2 no. 4), lalu perbarui baris Payroll di Bab 2.1 dan tambahkan tesnya.
5. **Project Budgeting** (4.2 no. 5).
6. **Royalty Dashboard** (4.2 no. 6).
7. **Upload CV dan link portofolio** (4.2 no. 7).
8. **Perbaiki bug dan keterbatasan yang diketahui** (4.3): kartu People/Recruitment di `/dashboard` dan pengajuan cuti tumpang tindih.
