...
| **Batch 4 — Import**       | Manajemen Karyawan → KPI → Project Budgeting (urutan segini: Karyawan duluan karena paling sering kepake buat onboarding massal)                                              | 🔶 **Manajemen Karyawan: kode selesai**, belum dites — KPI & Project Budgeting **belum dikerjakan**     |

Kartu **Import** untuk Work Tracker dan **Manajemen Karyawan** sekarang aktif (tombolnya sudah bisa diklik) — Work Tracker **sudah dites manual, aman**; Manajemen Karyawan **belum dites**, lihat catatan pengujian di bawah. KPI/Project Budgeting **masih "Segera Hadir"** — belum ditulis, menyusul lanjutan Batch 4. Semua tombol **Export** (Excel & PDF) yang ada di catalog sudah aktif, kecuali kartu yang memang cuma punya 1 format dari awal (lihat bagian 3).
...
### Batch 4 — Import Manajemen Karyawan (kode selesai, belum dites)

Beda dari Work Tracker (Batch 3) — import ini bikin **akun user baru** (bisa login), bukan cuma insert data biasa, jadi butuh keputusan tambahan dari Owner sebelum ditulis. Ini hasil finalnya, dan sudah diikuti persis di kode (`app/Imports/EmployeeImport.php`):
...
4. **Email yang sudah kepake** — baris itu **ditolak** (masuk baris error di preview, alasan "email sudah terdaftar"), TIDAK menimpa/update data karyawan lama. Kalau mau update data karyawan lama, tetap lewat halaman edit manual.
...
Urutan Batch 4 tetap: **Manajemen Karyawan** dulu (sudah dikerjakan sesuai rencana di atas — termasuk 1 pengaman tambahan yang gak dibahas eksplisit tapi perlu: 2 baris di file yang sama sengaja isi email baru yang SAMA, baris ke-2-nya ditolak juga, gak nunggu sampai bentrok pas nyimpan ke database), lanjut **KPI**, lalu **Project Budgeting** — masing-masing kemungkinan butuh keputusan sendiri-sendiri sebelum dikerjakan, sama seperti di atas.
...
**BELUM bisa dites langsung** (halaman preview tabel & tombol Download di semua modul Batch 1 + Batch 2, Excel maupun PDF; halaman upload/preview/konfirmasi Import di Batch 3 Work Tracker & Batch 4 Manajemen Karyawan): package `maatwebsite/excel` **dan** `barryvdh/laravel-dompdf` **sudah terpasang** di `vendor/` (dicek langsung foldernya ada, beda dari catatan sebelumnya yang bilang belum ter-install), tapi saya tetap **belum sempat login & coba beneran lewat browser** untuk Batch 1/2/4 (Batch 3 Work Tracker sudah dites manual sama Owner, lihat status di tabel batch). Jadi Batch 1/2/4 sudah diperiksa teliti secara **audit kode manual** (nama kolom & nama relasi tiap model dicocokkan satu-satu ke kode aslinya, bukan ditebak — termasuk breakdown slip gaji yang saya cocokkan ke `dashboard/payroll/show.blade.php`, notulen ke `dashboard/work/meetings/show.blade.php`, dan untuk Import Manajemen Karyawan: rules validasi dicocokkan ke migration `users` (enum `role`, `unique email`, `annual_leave_entitlement` min/max) & `StoreEmployeeRequest` yang sudah ada, plus pengaman ekstra buat 2 baris rebutan 1 email baru yang sama dalam 1 file) — tapi **belum dites "beneran jalan end-to-end"** oleh siapa pun untuk Batch 1/2/4. Kesimpulan audit: kodenya konsisten sama pola & kerangka Batch 0 (`BaseImport`/`ImportPreviewService`/`TemplateExport`) dan gak ada hal yang meleset dari model/rules aslinya sejauh yang saya periksa — tapi ini **bukan pengganti tes beneran**, ada kemungkinan gap yang cuma kelihatan pas benar-benar diklik (mis. kombinasi format tanggal dari Excel yang beda-beda, cocokan nama Project/PIC yang meleset dikit, atau — khusus Batch 4 — akun baru yang passwordnya gak sesuai ekspektasi). Tolong dicoba manual, kalau ada error tinggal kabari saya. Yang paling penting buat dicoba (urutan prioritas):
