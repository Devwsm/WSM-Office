...
> **Status kode (dicek ulang 2026-09-26 terhadap kode, riwayat git, dan log GitHub Actions; Bab 5 Production baru):** 383 tes otomatis lulus (4.185 assertion, dijalankan Arga lokal 2026-09-26, termasuk 3 tes baru `WorkReminderTest`), 160 route aplikasi (+ `/up`), 42 migrasi, 5 seeder. `migrate:fresh --seed` bersih di lokal yang sama. **Belum dites klik manual di browser** (production sudah berisi data asli, jadi verifikasi UI ditunda ke setelah deploy — lihat Bab 2.1 #28). **`phpunit.xml` sekarang mengunci `WOS_PREVIEW_MODE=true`** (2026-09-25), jadi nilai `WOS_PREVIEW_MODE` di `.env` lokal tidak lagi memengaruhi hasil tes (sebelumnya 376/380 kalau `.env` lokal berisi `WOS_PREVIEW_MODE=false`, lihat riwayat di 5.6). Isi dokumen ini dicocokkan dengan kode yang berjalan. Kolom "Prototype v32" di Bab 2.1 dicocokkan langsung ke source prototype (`WOS_2_0_STANDALONE_v32.html`) pada 2026-09-23; baris Bab 2.1 tidak diaudit ulang satu per satu pada 2026-09-26 (yang dicek ulang: baris #28, struktur file, dan butir yang ditandai sudah/belum dikerjakan).
...
**Ukuran:** 160 route aplikasi (75 GET, 85 tulis; + `/up`, health-check bawaan Laravel), 42 migrasi, 25 model, 32 FormRequest, 109 view Blade, 5 seeder, 383 tes (dijalankan Arga lokal 2026-09-26, semua lulus).
...
| 20  | Executive People Overview                                                                                  | Ada (label sidebar saja — tombolnya `data-opage="dashboard"`, sama persis dengan tombol "Dashboard")                                                                                             | Sudah ada, sama dengan Dashboard Owner (baris 15) — bukan halaman terpisah, cuma nama beda di sebagian sidebar prototype                                                                                                                                                                                                                                                                                                                                                                                        | ✅ (= baris 15, dicek langsung ke source prototype 2026-09-23)                                                                                                                                                                                   | Owner, Developer                             |
...
| 28  | Assign / Reminder dari dashboard                                                                           | Ada — bagian dari **Work Dashboard** (Secretary/Admin Console), akses khusus jabatan tertentu: assign task, follow-up tracker, blast memo, dan reminder pekerjaan (reminder ikut masuk ke Memos) | Ada — form "Kirim Reminder" di atas listing MoM & Memo (`dashboard.work.index`), gated `module:work,manage`. 1 submit lewat `WorkReminderController` bikin `WorkItem` (`is_reminder=true`, section "REMINDER / ADMIN", masuk board Work Tracker) **dan** `Memo` bertarget (`audience=tertentu`) ke karyawan yang sama sekaligus — padanan langsung `sendAdminReminder`/`sendAdminDashboardReminder` prototype. Blast memo ke banyak orang tetap lewat form Memo biasa (`MemoController`), bukan lewat form ini. | ✅ **(2026-09-26; 383/383 tes lulus termasuk 3 baru `WorkReminderTest`, `migrate:fresh --seed` bersih. Klik manual di browser belum — production sudah ada data asli, ditunda ke setelah deploy, pakai akun sendiri sebagai target percobaan.)** | Modul `work` (manage)                        |
...
**Hitungan status (50 baris):** ✅ 31 · ⚠️ 8 · ❌ 4 · ➕ 7.
...
**Menjalankan:** `php artisan test` (SQLite `:memory:`; `public/hot` dan `public/build` tidak perlu ada; `phpunit.xml` sudah mengunci `WOS_PREVIEW_MODE=true`, jadi nilai di `.env` lokal tidak berpengaruh, lihat 5.6). Untuk MySQL/MariaDB: buat database kosong khusus tes lalu `DB_CONNECTION=mysql DB_DATABASE=wsm_test DB_USERNAME=... DB_PASSWORD=... php artisan test` — jangan arahkan ke database aplikasi, `RefreshDatabase` menghapus isinya.
...
│   │   │       └── WorkReminderController.php     # README #28: 1 submit -> WorkItem(is_reminder) + Memo tertarget
│   │   ├── Employee/
...
└── database.sqlite                         # Sisa setup awal (koneksi aktif MySQL); di-.gitignore, tidak ikut ke-checkout runner — aman dibiarkan, tidak perlu dihapus
...
routes/
├── web.php                                 # 160 route aplikasi (+ /up): publik, /app, /owner, /dashboard, rekrutmen, approval, system/deploy-hook
...
8. **Popup informasi preview (tahap testing).** Kecil dan tidak menyentuh paket, jadi dikerjakan **lebih dulu** dari butir lain. Selama masih tahap pengembangan, tiap pintu masuk menampilkan modal yang memberi tahu bahwa sistem belum bisa dipakai untuk operasional, data yang masuk hanya untuk testing, dan data itu akan di-reset sebelum sistem dipakai operasional. **Status: selesai (2026-09-22)** — 380/380 tes lulus (10 tes baru di `EntryPopupsTest`), `npm run build` sukses.
    ...
    - **Saklar (`.env`):** `WOS_PREVIEW_MODE` (default `true`) mematikan Peringatan preview di semua pintu sekaligus; wajib di-set `false` saat sistem mulai dipakai operasional (5.7 no. 2). `WOS_ENTRY_POPUPS` (default `true`) mematikan semua popup; `phpunit.xml` mengisinya `false` supaya tes yang ada (383, termasuk `WorkReminderTest` baru) tidak berubah.
...
9. ✅ **Selesai (2026-09-26).** Assign / Reminder dari dashboard: form "Kirim Reminder" di `dashboard.work.index` (`module:work,manage`) — 1 submit bikin `WorkItem` (`is_reminder=true`, section "REMINDER / ADMIN", langsung kelihatan di board Work Tracker) **dan** `Memo` tertarget ke karyawan yang sama, lewat `WorkReminderController` baru. Blast memo ke banyak orang tetap pakai form Memo biasa yang sudah ada. Detail di Bab 2.1 no. 28. 383/383 tes lulus lokal (termasuk 3 baru `WorkReminderTest`); klik manual di browser ditunda ke setelah deploy karena production sudah berisi data asli.

**Sudah tidak perlu keputusan lagi (diklarifikasi 2026-09-23 dari source prototype yang sekarang ada):**

- **Executive People Overview** (Bab 2.1 no. 20): ternyata di prototype ini cuma label sidebar lain untuk halaman Dashboard yang sama (`data-opage="dashboard"`), bukan halaman terpisah. Sudah ✅, tidak perlu dibuatkan apa-apa lagi.
- **Assign / Reminder dari dashboard** (Bab 2.1 no. 28): sudah dikerjakan 2026-09-26, lihat Bab 4.2 no. 9 (selesai).

**Belum dijadwalkan (masih butuh keputusan atau penjelasan):**
...
7. ✅ **Assign / Reminder dari dashboard** (4.2 no. 9): selesai 2026-09-26, 383/383 tes lulus lokal. Klik manual di browser masih ditunda (production sudah ada data asli).
...
- **Riwayat versi workflow:** 2026-09-22 pipeline pertama (tes → build → patch `index.php` → `lftp` ke dua akun FTP) → 2026-09-23 `reset` → workflow sekarang (satu akun FTP, `FTP-Deploy-Action`) → 2026-09-23 `vendor/` dikeluarkan dari upload (5.8 no. 2) → 2026-09-24 `curl` hook kembali memverifikasi SSL (5.8 no. 3) → 2026-09-25 kedua step upload FTP dibungkus retry otomatis (5.8 no. 1) dan `exclude` app-upload ditambah 9 file dev (Blocker 8).
...
- ✅ **Selesai (2026-09-25):** dulu tes otomatis ikut membaca `WOS_PREVIEW_MODE` dari `.env` lokal — kalau `.env` lokal berisi `WOS_PREVIEW_MODE=false` (meniru production), 4 tes `EntryPopupsTest` gagal (376 dari 380 lulus, dicek 2026-09-24). Sekarang `phpunit.xml` mengunci `<env name="WOS_PREVIEW_MODE" value="true"/>`, jadi ini tidak terjadi lagi apa pun isi `.env` lokal.
...
| 8   | **Bersihkan paket upload**            | File dev/pribadi tidak boleh ikut ke server.                                                                                                         | `exclude` di step "Upload kode aplikasi ke ~/wsm-office" sudah ditambah (2026-09-25): `AGENTS.md`, `CLAUDE.md`, `README.md`, `phpunit.xml`, `package.json`, `package-lock.json`, `vite.config.js`, `.editorconfig`, `.npmrc` tidak lagi ter-upload mulai push berikutnya.                                                                                                                                                                              | ✅ **selesai untuk push berikutnya**; salinan lama yang sudah terlanjur ada di `~/wsm-office` dari deploy sebelumnya perlu dihapus manual sekali (5.10) — tidak bisa dibuka dari web (di luar `public_html`), jadi tidak darurat |
...
Semua yang di bawah ini **sudah otomatis dikecualikan** oleh `exclude` di `deploy.yml` (langsung lewat blanket-exclude `storage/`, `public/`, `.git*`, `node_modules/`, `tests/`, atau lewat entri eksplisit sejak 2026-09-25) — tidak perlu tindakan manual per file lagi:

- `.git/`, `node_modules/`, `tests/`
- `.env` lokal (server pakai `.env` produksi sendiri)
- `storage/logs/laravel.log`, cache Blade di `storage/framework/views/`, dan isi `storage/` lainnya
- `public/hot`, `public/fonts-manifest.dev.json`, dan isi `public/` lain di luar `public/build/` (di-upload terpisah lewat step kedua)
- `AGENTS.md`, `CLAUDE.md`, `README.md`, `phpunit.xml`, `package.json`, `package-lock.json`, `vite.config.js`, `.editorconfig`, `.npmrc`

`database/database.sqlite` dan `.phpunit.result.cache` **tidak perlu dikhawatirkan** — keduanya di-`.gitignore` (lihat `database/.gitignore` dan root `.gitignore`), jadi tidak pernah ikut ke-checkout oleh runner sama sekali, apa pun isi `exclude` di workflow.
...
| --- | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | 2026-09-23 sore → berulang (terakhir run #21, 2026-09-24) | Step upload FTP gagal `Error: Timeout (control socket)` — muncul lagi di run #17–#19, run #20 percobaan pertama, dan run #21 percobaan pertama, walau `FTP_HOST` sudah IPv4. Step gagal dalam ±2 detik (30 detik total step, gagal cepat di awal). | **Bukan masalah di sisi kita.** Stack trace menunjuk `internalConnectMultipleTimeout` (timer pemilihan alamat IPv4/IPv6 bawaan Node), bukan timeout koneksi sungguhan. Ini bug flaky yang sudah lama dikenal di `SamKirkland/FTP-Deploy-Action` sendiri (banyak laporan serupa "Error: Timeout (control socket)" di issue tracker-nya sejak 2020, belum ada fix resmi dari mereka). IPv4 di `FTP_HOST` mengurangi kemunculannya tapi **tidak menghilangkannya**. | Sebelum 2026-09-25: re-run job manual (selalu berhasil di percobaan berikutnya). **Sejak 2026-09-25:** kedua step upload FTP di `deploy.yml` sudah dibungkus retry otomatis (percobaan 1 gagal → percobaan 2 jalan sendiri tanpa perlu re-run manual). |
...
| -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Step upload gagal `ETIMEDOUT`/`Timeout (control socket)`, **±2 detik**, tapi run keseluruhan tetap hijau | Pemilihan alamat IPv4/IPv6, bug flaky dari `FTP-Deploy-Action` (5.8 no. 1)           | Tidak perlu tindakan — sejak 2026-09-25 step retry otomatis yang menyelamatkan (cek di log: step "percobaan 1" merah, step "retry" hijau).                                                                                                                      |
| Step upload gagal `ETIMEDOUT`/`Timeout (control socket)` di **percobaan 1 maupun retry**, run merah      | Kemungkinan bukan sekadar flaky Node lagi — bisa jadi masalah jaringan/FTP sungguhan | Pastikan `FTP_HOST` berisi alamat IPv4, bukan hostname. Re-run manual sekali lagi. Kalau masih gagal, lanjut ke baris "±30 detik" di bawah.                                                                                                                     |
